 ...
 * lib/goalify/video.ts) when one resolves and actually loads. Falls back
 ...
 * bucket, offline) — never a broken video icon or dead space. Kept
 * deliberately plain — no corner brackets, scan sweep, or overlaid label —
 * the caller (LivePlayer) already puts that context in one small badge and
 * in the phase/headline directly below, so this stays a clean image/video.
 */
export function AIFormGuide({
  ...
  return (
    <div
      className={clsx(
        "gf-slot relative flex flex-col items-center justify-center overflow-hidden",
        className,
      )}
    >
      {attemptingVideo && (
