...
## 4א. הזיכרון - איך המערכת לומדת מהעסק עצמו

כל אבחון נשען על הלולאות הסגורות של אותו עסק, לא רק על הסימנים של השבוע.
בכל צ'ק-אין `POST /api/businesses/[id]/checkin` נטענות ההחלטות שנשלחו לעסק
יחד עם התוצאות שדווחו עליהן, והן נכנסות לפרומפט של `lib/diagnosis/generate.ts`.

- `lib/diagnosis/history.ts` מקפל את ההיסטוריה ל-Track Record: כמה מהלכים
  באמת נמדדו (בוצעו **וגם** דווחה עליהם תוצאה), כמה עבדו, פילוח לפי ערוץ,
  ורצף המהלכים האחרונים שלא בוצעו.
- **מהלך שלא בוצע אינו הפרכה של האבחנה** - הוא עדות על חיכוך ביצוע בלבד.
  לכן הוא לא נספר ב-`measured`, ורצף של שניים כאלה מורה למודל להציע מהלך
  קטן וקצר יותר.
- `decision.learned_from_prior` הוא המשפט שמוצג ללקוחה ומסביר מה ההחלטה
  לוקחת מההיסטוריה שלה. הוא נאכף בקוד, לא רק בפרומפט: בלי היסטוריה כלל
  הוא נכפה ל-`null`, ובתור האישורים של המנהל שדה העריכה בכלל לא מוצג -
  כדי שלא ייווצר "ניסיון קודם" יש מאין.
- **כיול ביטחון**: `HIGH` מותר רק כשלפחות שני מהלכים שבוצעו הביאו תוצאה
  חיובית אצל אותו עסק. אחרת הוא מורד ל-`MEDIUM` בקוד, גם אם המודל ביקש אחרת.
